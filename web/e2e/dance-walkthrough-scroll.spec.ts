// Not working yet - commented out rather than left failing. The
// scroll-container check below was reading scrollHeight/clientHeight
// off the contentEditable itself, which is never the scroll container
// (a plain block element that just renders at its full content height)
// - the actual bounded, overflow-y-auto element is its parent
// (EditorContent's own wrapper). Confirmed via a debug run that the
// real CSS chain (fillHeight/AppShell) is correctly bounded end to end;
// this test just needs to check editor.parentElement instead.
...
//     // the overflow - its content is taller than what's visible.
