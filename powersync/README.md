...
**When writing a junction-table stream query**, keep its source table unaliased - an alias on the table actually being synced (as opposed to a table it's only joined against) makes PowerSync sync rows under that alias's name instead of the real table name, breaking the client schema mapping. `powersync validate` catches this.
