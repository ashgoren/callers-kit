...
`sync-config.yaml` defines ten streams, all auto-subscribed: `dances` and `programs` (`SELECT *`, scoped per-user via `WHERE ....user_id = auth.user_id()`); `choreographers`, `key_moves`, `vibes`, and `user_table_preferences` (same per-user pattern, each with its own `user_id` column); and `dance_versions`, `dances_choreographers`, `dances_key_moves`, `dances_vibes`, and `programs_dances` (tables with no `user_id` of their own - each scoped via a join to the owning dance's `user_id` instead). `dance_type`/`formation`/`progression` are plain enum columns on `dances` itself (not a joined lookup table), so they have no stream of their own.
